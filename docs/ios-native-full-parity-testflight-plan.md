...
- `apps/native/app.json`의 committed `ios.buildNumber`는 다음 제출 baseline이며, 새 TestFlight 제출이나 재제출 전에는 App Store Connect에 이미 올라간 마지막 값보다 1 크게 수동으로 올린다. EAS `autoIncrement: true`는 빌드 파이프라인 내부 증가용이라 재시도 중복 번호를 막아주지 않는다.
- 최신 TestFlight 제출은 `2026-06-19` main SHA `667aafeb4546eb015a9ef7894f6cba9183db043e`, GitHub run `27805741470`, EAS build `7796ef11-75c1-4acb-95d7-96018e10bdbc`, App Store Connect build `1.0.0 (7)`이다.
- 남은 작업은 TestFlight 설치 빌드 기준 iPhone 실기기 QA와 웹/Android 앱 대비 기능, UI/UX, 문구 parity 안정화다.
...
- 진행 중: App Store Connect TestFlight build processing/available 확인
- 진행 중: 내부 테스터 설치와 iPhone 실기기 QA
...
- App Store Connect build number: `1.0.0 (7)`
- TestFlight processing/available 상태: Apple processing 완료 후 App Store Connect TestFlight 탭에서 확인
