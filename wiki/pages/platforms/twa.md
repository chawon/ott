...
- 현재 운영 상태(2026-04-21):
  - `versionName=1.0.3`, `versionCode=7` AAB 빌드 완료
  - Play `alpha` 트랙에 `7 (1.0.3)` 반영 완료
  - 홈 화면 위젯 디자인 리프레시 반영
  - Google Play `alpha` 비공개 테스트(Closed testing) 시작
  - 비공개 테스트 테스터 그룹: `ottline-beta-testers@googlegroups.com`
- 다음 후보(2026-04-29):
  - 런처 롱프레스 shortcuts 4종 (`기록하기`, `영상 기록`, `책 기록`, `타임라인`)
  - PWA/TWA Web Share Target 명시 (`shared_text`, `shared_subject`, `shared_url`)
...
- 2026-04-21 기준 최신 반영:
  ...
  - `alpha`: `7 (1.0.3)`, `status=completed`
  - `alpha` 비공개 테스트 시작
...
2026-04-29 후보에서는 PWA manifest와 TWA metadata에 Web Share Target을 명시하고, URL 공유용 `shared_url`도 홈 QuickLog 입력으로 병합한다.
...
## Android 런처 Shortcuts (2026-04-29 후보)
