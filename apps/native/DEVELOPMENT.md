...
상위 실행 계획은 `docs/ios-native-full-parity-testflight-plan.md`를 기준으로 한다. App Store 심사 제출 준비는 `docs/ios-app-store-submission-prep.md`를 기준으로 한다.

## 현재 범위

- 현재 목표: TestFlight build `1.0.0 (14)` 기준 iPhone 실기기 QA와 웹/Android parity 안정화를 진행하고, `2026-07-01` 이후 build `1.0.0 (15)` 재시도 전 App Store 심사 제출 준비를 끝낸다.
...
- UGC 운영: 공개 글/댓글은 App Review Guideline 1.2 대응이 필요하다. 현재 iOS 공개 상세에는 신고/차단 UX가 없으므로 App Store 제출 전 P0로 정리한다.
...
- EAS Free plan iOS build quota reset 이후 build `1.0.0 (15)` 재시도
- TestFlight build processing/available 상태를 App Store Connect TestFlight 탭에서 확인
- 내부 테스터가 TestFlight에서 build `1.0.0 (15)`을 설치할 수 있는지 확인
...
현재 기준 최신 QA 제출은 build `1.0.0 (14)`이며, 다음 목표는 build `1.0.0 (15)`다. `2026-06-21` GitHub run `27902317186`은 EAS Free plan iOS build quota exhausted로 build/upload 전 실패했다. 재시도 기준 SHA는 `76d82da9526d8ce97b7c09dda65286eef1479f26`이다.
...
4. GitHub Actions `Native iOS TestFlight` workflow를 병합된 main SHA로 수동 실행
   - 현재 workflow는 `eas.json`의 `autoIncrement: true`로 실제 build number를 한 번 올린다.
   - build `1.0.0 (15)` 재시도 전에는 `apps/native/app.json`의 `ios.buildNumber`를 `14`로 유지한다.
   - 향후 build를 올릴 때는 "committed baseline + EAS autoIncrement 결과"가 App Store Connect의 마지막 uploaded build보다 커지는지 확인한다.
...
App Store Connect 입력 초안과 reviewer 접근 설명은 `docs/ios-testflight-review-notes.md`와 `docs/ios-app-store-submission-prep.md`에서 관리한다.
