...
28. 2026년 상반기 리캡: 개인 이용 리포트에 `2026-H1` 리캡 데이터를 추가하고, 웹/네이티브 리포트에서 포스터 중심의 상반기 돌아보기와 SNS 공유용 카드 생성을 지원한다. 웹은 KST 기준 `2026-07-01`부터 `2026-07-31`까지 상단 바 아래에 `2026년 상반기를 돌아볼까요?` 노티를 노출해 리포트로 진입시켰다(`2026-07-01` production 반영, web/api SHA `9df9cb0`). 캠페인 종료에 맞춰 KST `2026-08-01 00:00`부터 API `seasonalRecap`과 웹 리포트·로컬 fallback의 리캡 노출을 종료했다(`2026-08-03` production 반영, web/api SHA `5eb5115`).
29. 현실적인 관리자 통계 집계: 웹 `app_open`을 브라우저 탭 세션당 한 번으로 제한하고, 계정·클라이언트·세션을 안전하게 연결한 resolved actor 기준의 행동 사용자와 세션 기준 실행량을 추가했다. 관리자 화면과 데일리 리포트는 기존 DAU/WAU/MAU 및 순차 전환율 대신 행동 사용자·활성 클라이언트·앱 실행 세션·원본 실행 이벤트와 독립 행동 도달을 사용한다(`2026-07-14` production 반영, web/api SHA `b06bf4c`). Cloudflare 데일리 통계는 UTC 날짜 버킷 대신 KST 전일을 UTC 시간 범위로 변환해 HTTP·RUM을 같은 `[start, end)` 구간으로 집계한다(`2026-08-03` production 반영, api SHA `12ed846`). 후속 배포에서는 RUM `visits`를 고유 방문자가 아닌 `방문`으로 바로잡고, GA4 전일 값은 잠정치, Kubernetes는 조회 시점 스냅샷, iOS `app_store_testflight`는 설치 출처 미구분으로 표시했다(`2026-08-03` production 반영, web/api SHA `5c70b2b`).
30. SEO·검색 유입 기반과 관리자 접근 경계: 한국어·영어 공개 페이지의 canonical/hreflang/OG, 검색 가이드 6개, 공개 URL 전용 sitemap/robots/llms.txt, 배포 후 IndexNow, 유입 세션 집계와 180일 보존 정책을 반영했다. `/admin/**`는 Cloudflare Access와 origin JWT 재검증으로 보호하고 PWA 캐시에서 제외했다(`2026-07-19` production 반영, web/api SHA `8303f4f`). `2026-07-25`에는 네이버 검색어 후속 점검을 바탕으로 `/about`과 영화·시리즈 가이드의 검색 문구, 가이드 직접 링크, 한국어 스토어 링크, `SoftwareApplication`의 공식 스토어 연결을 보강했다(web SHA `524b001`). iOS 바이너리는 새로 빌드하거나 배포하지 않았다.
31. KDC 기반 로컬 우선 내 서가 v1: 한국어 화면에서 사용자가 남긴 책을 ISBN 판본 기준으로 중복 제거하고 한국십진분류법 0~9 대분류 서가에 자동 정리한다. QuickLog 저장은 분류 장애와 관계없이 유지하고, 분류 결과는 PostgreSQL 공용 캐시와 IndexedDB 로컬 캐시에 저장한다. 타임라인 책 필터의 작은 진입점, `/me/bookshelf`, 분류 대기 영역, 기록 카드 KDC 배지를 제공한다(`2026-07-29` production 반영, web/api SHA `35b6263`).
32. Android Play 유입 활성화 1차: Android 브라우저와 설치형 PWA에는 기존 PWA 설치 대신 Google Play 전환 팝업을 노출하고, ottline Google Play TWA 세션은 제외한다. TWA에서 첫 기록이 생긴 사용자에게는 시청 기록 알림 설정 카드를 노출하며, 두 UI는 닫기 또는 이동 뒤 14일간 다시 노출하지 않는다. TWA 판정은 정확한 `android-app://app.ottline` referrer, 현재 launch URL의 앱 버전·install token, 그 신호로 기록한 현재 세션 플래그만 사용한다(`2026-08-18` production 반영, web SHA `57f98e6`).
33. Android 활성화 2차 및 Play 권장 조치 대응: 네이티브 시청 기록 알림 설정을 현재 필요한 한 단계 중심으로 정리하고 Android 15 edge-to-edge/inset 처리, Android Browser Helper `2.7.2`, AGP `9.3.1`, R8 코드·리소스 축소를 반영했다. 최소 지원 버전은 Android 6.0(API 23)이다. 시작 크래시 안정화 과정에서 R8 코드 최적화만 임시로 껐으며 축소·난독화와 리소스 축소는 유지한다.
...
38. 타임라인 서버 커서 무한 스크롤: 웹/PWA/Android TWA 타임라인이 `GET /api/logs/page`의 `nextCursor`로 처음 50개 이후 기록을 50개씩 이어서 조회하고 IndexedDB에 캐시한다. 기존 배열 `GET /api/logs`와 iOS 네이티브의 `/api/sync/pull` 계약은 유지한다. `2026-09-15` PR `#107`, API/Web SHA `c0e09c4`로 배포했으며 ArgoCD `Synced Healthy`, 두 Pod `1/1` ready·restart 0, 운영 새/기존 로그 계약 200과 실행 번들의 신규 경로·문구를 확인했다. 사용자가 운영 앱에서 50개 이후 기록이 정상 노출되는 것을 직접 확인했다. 상세 결과는 `docs/timeline-infinite-scroll.md`를 따른다.
39. 이용 리포트 주간 공유 카드 포스터 복구: 개인 리포트 API와 로컬 fallback이 KST 기준 직전 주 기록에서 작품별 포스터 후보를 최대 6개 만들고, 시즌 포스터를 기본 포스터보다 우선한다. 웹 공유 payload가 후보를 전달하며 서버 렌더러는 후보 수에 맞춘 모자이크와 이미지가 없는 작품의 제목 fallback을 사용한다. `2026-09-21` PR `#108`, API/Web SHA `01ef6b9`로 배포했고 ArgoCD `Synced Healthy`, 두 Pod `1/1` ready·restart 0, 운영 리포트 계약과 실제 1080×1920 PNG의 포스터 렌더를 확인했다. 상세 결과는 `docs/weekly-report-share-card-posters.md`를 따른다.
...
### 취소
1. **OCI Always Free 축소 대응 및 인프라 right-sizing**: `2026-08-03` OCI 계정이 paid 계정임을 확인해 `2 OCPU / 12GB` 강제 축소 전제가 사라졌다. DB와 OKE 리소스 축소는 진행하지 않고 현재 구성을 유지한다. `2026-06-15`에 완료한 `n8n`·상시 staging 제거와 production 수동 배포 전환은 그대로 유지한다. 당시 검토 기록은 `docs/oci-always-free-rightsizing.md`에 종료 상태로 보존한다.
...
16. **Android production analytics 세그먼트 보강**: Google Play TWA 접근은 정확한 ottline Android referrer, 현재 launch URL의 앱 버전·install token, 그 신호로 기록한 현재 세션 플래그로만 `platform=twa`로 분류한다. 일반 Android WebView, standalone PWA, 과거에 저장된 앱 컨텍스트 자체는 TWA 판정 근거에서 제외하고, 관리자 analytics overview에는 Android 앱 버전, versionCode, TWA 판별 신호 세그먼트를 표시한다. 초기 세그먼트는 `2026-06-06` PR `#49`, main SHA `fa528c2f82a4ded2c02fac97919155add2a61f5f`로 배포했고, 현재 판정 기준은 `2026-08-18` PR `#93`, main SHA `57f98e6317dcce303fc9d32e488b546a68b9c2c6`에서 강화했다.
...
21. **2026년 상반기 리캡 및 헤더 노티**: 개인 이용 리포트 API에 `seasonalRecap`을 추가해 KST 기준 `2026-01-01`부터 `2026-06-30`까지의 상반기 기록 수, 대표 장르, 대표 상태, 포스터 후보를 반환한다. 웹/네이티브 리포트에는 `2026년 상반기 돌아보기` 섹션과 포스터 중심의 SNS 공유 카드 payload를 추가했고, 웹/네이티브 모두 로컬 기록 기반 fallback 리캡을 지원한다. 웹은 `/og/share-card` 서버 렌더 경로를 추가하고, KST 기준 `2026-07-01`부터 `2026-07-31`까지 상단 바 아래에 `2026년 상반기를 돌아볼까요?` 노티를 노출한다. `2026-07-01` PR `#75`, main SHA `9df9cb05addf90d56a82042b5393baa29fe78349`, API production run `28494192890`, web production run `28494344708`, API manifest commit `048431dad4fe1ba9045e3db52b82d078e86849a9`, web manifest commit `5bc388385839fc08fcf7a16fd01c2f690904e156`로 배포 완료했고 ArgoCD `ott-app` `Synced Healthy`, production `ott-api`/`ott-web` 이미지 태그 `9df9cb05addf90d56a82042b5393baa29fe78349`, `APP_VERSION=9df9cb0`을 확인했다. iOS 네이티브 코드는 main에 병합됐지만 실제 App Store 사용자 배포는 별도 TestFlight/App Store 업데이트가 필요하다. `2026-08-03`에는 캠페인 종료에 맞춰 KST `2026-08-01 00:00`부터 API가 `seasonalRecap=null`을 반환하고 웹 리포트와 로컬 fallback에서도 리캡을 숨기도록 PR `#86`, main SHA `5eb5115317892d0032a8b7b6fe5377b14f2d0c2a`로 종료 배포했다. API/Web main CI run `30778332152`/`30778332125`, API/Web production run `30778501095`/`30778595558`, API/Web manifest commit `abb40162543507255e2a00cbdacd89a577009ed1`/`a68753be1dceb50cc5cf1f17cb63f733b7d86de9`를 확인했다. ArgoCD `ott-app`은 `Synced Healthy`, production `ott-api`/`ott-web` 이미지와 `APP_VERSION=5eb5115`, Pod `1/1` ready·restart 0이었고 사용자 화면에서도 정상 종료를 확인했다. 이 종료 배포에서는 iOS 네이티브 코드와 App Store 바이너리를 변경하지 않았다.
...
26. **Android Play 유입 활성화 1차**: Android 브라우저와 설치형 PWA에는 Google Play 전환 팝업을, ottline TWA에서 첫 기록이 있는 사용자에게는 시청 기록 알림 설정 카드를 노출한다. 닫기·이동 후 14일 cooldown, ko/en 개인정보 고지, 정확한 ottline TWA 판정, Play CTA·알림 카드의 노출/클릭/닫기 6개 analytics 이벤트를 포함한다. `2026-08-18` PR `#93`, main SHA `57f98e6317dcce303fc9d32e488b546a68b9c2c6`, PR/main Web CI run `32099214004`/`32099337243`, Web production run `32099474936`, manifest commit `2b4a433cf2f001dd233f459aa5e34d5499575d67`로 배포했다. ArgoCD `ott-app` `Synced Healthy`, production `ott-web` 이미지와 `APP_VERSION=57f98e6`, Pod `1/1` ready·restart 0, 실행 번들의 신규 이벤트와 한국어 CTA 문구를 확인했다. API, DB, Android AAB, iOS 네이티브 바이너리는 변경하거나 배포하지 않았다.
27. **Android 활성화 2차 및 Google Play 권장 조치 대응**: 시청 기록 알림 설정 화면을 현재 필요한 한 단계 중심으로 정리하고, `EdgeToEdge.enable`과 `WindowInsetsCompat`로 Android 15 system bar/cutout inset을 처리했다. 앱의 system-bar color 직접 호출을 제거하고 Android Browser Helper를 `2.7.2`로 올렸다. AGP `9.3.1`·Gradle `9.5.0`과 `optimization { enable = true }`로 R8 코드 최적화·난독화·축소·optimized resource shrinking을 활성화했으며 `minSdk=23`으로 전환했다. `2026-08-18` PR `#94`, main SHA `982e5bc8330069b7b75a4f220ccab0d08419d3e2`, main AAB run `32117048949`, Google Play release run `32117645033`으로 internal `1.0.14` (`versionCode=18`, `status=completed`) 등록을 확인한 뒤 edit `06048068128972830965`로 beta에도 같은 버전을 승격했다. production은 `1.0.13` (`versionCode=17`, `status=completed`)을 유지한다. 실제 단말과 Play 권장 조치 재분석은 후속 확인한다.
28. **타임라인 서버 커서 무한 스크롤**: 기존 첫 50개 제한을 해소하기 위해 `updatedAt|watchedAt + UUID` 키셋 커서를 사용하는 `GET /api/logs/page`를 추가하고, 웹/PWA/TWA가 하단 도달 시 다음 50개를 받아 IndexedDB에 저장하도록 반영했다. 기존 `/api/logs` 배열과 iOS 네이티브 `/api/sync/pull`은 그대로 유지한다. `2026-09-15` PR `#107`, main SHA `c0e09c4bf90f2d6de11a1f7c715dc25c3a48c27e`, API/Web production run `34912864768`/`34913130614`, manifest commit `bb3101e43a0bccdc80f1a3bdc90ee680cca43404`/`4aa7e501fed9405d5386832c2e56834d709028cd`로 배포했다. ArgoCD `ott-app` `Synced Healthy`, production API/Web 이미지와 `APP_VERSION=c0e09c4`, Pod `1/1` ready·restart 0, 운영 페이지·기존 배열 계약 200과 실행 번들의 `/logs/page` 및 ko/en 문구를 확인했다. 사용자가 운영 앱에서 50개 이후 기록이 정상 노출되는 것을 직접 확인했다. 네이티브 바이너리는 변경하거나 배포하지 않았다.
29. **이용 리포트 주간 공유 카드 포스터 복구**: `GET /api/nalytic/me/report`에 `previousWeekPosters`를 추가하고, KST 직전 주의 작품을 중복 제거해 포스터 유무·평점·기록 수·최근 기록 순으로 최대 6개 선정한다. 시즌 포스터를 우선하며 웹 로컬 리포트도 같은 규칙을 사용한다. 주간 공유 payload와 `/og/share-card` 렌더러는 후보 수별 1~6칸 모자이크를 만들고 이미지가 없는 후보는 제목으로 채운다. `2026-09-21` PR `#108`, main SHA `01ef6b9b5b018970006ab6f42118b46824fb3430`, API/Web production run `35569268524`/`35569520338`, manifest commit `4cb3d731616df253ee5d60b9199e64e8bf506e94`/`ef40081a1a188315f3b8a15027a5a5f4f1e4531f`로 배포했다. ArgoCD `ott-app` `Synced Healthy`, production API/Web 이미지와 `APP_VERSION=01ef6b9`, Pod `1/1` ready·restart 0을 확인했다. 운영 내부 스모크 테스트에서 리포트 계약 200, TMDB 포스터가 포함된 1080×1920 PNG 200과 실제 이미지 렌더를 확인했고 임시 검증 계정은 즉시 삭제했다. iOS 네이티브 소스와 App Store 바이너리는 변경하거나 배포하지 않았다.
...
2. 현재 기준
   ...
   3. `2026-08-20` 기준 Google Play `beta`와 `production` 최신 버전은 모두 `1.0.18` (`versionCode=22`, `status=completed`)이고, `internal`은 `1.0.14` (`versionCode=18`, `status=completed`)이다. 시청 기록 알림은 Usage Access 기반으로 대상 OTT 앱 사용 종료 후 기록 알림을 띄우며 콘텐츠 제목·재생 상태·화면 내용은 읽지 않는다. 다음 Play 업데이트는 `versionCode=23` 이상을 사용한다.
   4. Google Play 프로덕션 액세스는 `2026-06-06` 기준 심사 통과했다. 같은 날 `gplay promote --package app.ottline --from alpha --to production --status completed --rollout 1.0`로 alpha `1.0.9`(`versionCode=13`)를 production 트랙에 등록했고, `gplay tracks get`으로 production `1.0.9`/`13`/`completed` 반영을 확인했다. Production 릴리스 노트는 `Adds Apple TV as a supported watch reminder app.`로 정리했다. 이전 Play 검토 반려 사유는 비공개 테스트 중 실제 테스터 참여가 없었고, 사용자 의견 수집/조치/앱 업데이트 같은 테스트 권장사항을 충족하지 못했다는 점이었다. `alpha` 테스터 그룹은 `ottline-beta-testers@googlegroups.com`가 연결돼 있다.
   ...
   6. `2026-05-21` 기준 `1.0.6` (`versionCode=10`)을 Google Play `alpha`에 업로드해 비공개 테스트를 다시 시작했다. 12명 이상 테스터가 모두 opt-in한 시점부터 14일 연속 조건을 계산했으며, `2026-06-06` 기준 14일 조건 완료 후 프로덕션 심사 통과까지 진행했다.
   ...
   11. `2026-06-07` 기준 OTT 시청 감지 불안정성 이슈를 해결하기 위해 자동 감지의 pendingCandidate 체크 누락 수정, AUTO_CANDIDATE_WINDOW_MS 2시간 연장, queryUsageSnapshots의 48시간 누적 집계 방식에서 최신 단일 버킷 참조 방식으로 전환을 반영했다. PR `#57` main SHA `18e42e7`로 병합했고, TWA release run `27090728553`으로 Google Play `beta` `1.0.11` (`versionCode=15`, `status=completed`) 반영을 확인했다.
   ...
   16. `2026-08-18` Android Play 유입 활성화 1차 Web 배포를 완료했다. Android 브라우저·설치형 PWA의 Play 전환 팝업과 ottline TWA의 시청 기록 알림 설정 카드를 추가했고, 일반 WebView·standalone PWA·다른 패키지 TWA는 ottline TWA로 판정하지 않는다. PR `#93`, main SHA `57f98e6317dcce303fc9d32e488b546a68b9c2c6`, Web production run `32099474936`, manifest commit `2b4a433cf2f001dd233f459aa5e34d5499575d67`이며 Android AAB는 변경하지 않았다.
   17. `2026-08-18` Android 활성화 2차와 Google Play 권장 조치 대응을 PR `#94`, main SHA `982e5bc8330069b7b75a4f220ccab0d08419d3e2`로 반영했다. main AAB run `32117048949`와 release run `32117645033`이 성공했고, edit `06048068128972830965`로 beta 승격 후 별도 Play edit 조회에서 internal·beta `1.0.14`/`18`/`completed`, production `1.0.13`/`17`/`completed`를 확인했다. 최소 지원 버전은 Android 6.0(API 23)이며 실제 단말 검증과 Play 권장 조치 재분석 확인이 남아 있다.
   18. 같은 날 SM-S937N/Android 16에서 beta `1.0.14`가 splash 전 종료됐고, 선택적 알림 초기화 예외를 격리한 PR `#95` main SHA `ab607fe0508c4a59f10d60a66caf0fe9914277a8`, release run `32139664251`, beta `1.0.15` (`versionCode=19`)에서도 동일했다. PR `#96` main SHA `435b2a85ab1d83191006a4f540a3e15e6bc57b63`은 R8 축소·난독화는 유지하고 `-dontoptimize`로 코드 최적화만 껐다. 수동 AAB run `32145077156`에서 R8 메타데이터 `shrinking=true`, `obfuscation=true`, `optimizations=false`를 확인했고, release run `32146423912`로 beta `1.0.16` (`versionCode=20`, `status=completed`)을 배포했지만 같은 기기에서 동일 종료됐다.
   19. Play Vitals에서 `WorkManagerInitializer`가 `androidx.work.impl.WorkDatabase_Impl.<init>()`을 찾지 못한 스택을 확인했다. PR `#98`, main SHA `ef9f6a9fe459cd62a23127dbd4c284aa4076e021`은 해당 생성자만 보존하고 실제 release DEX를 검사하는 CI 게이트를 추가했다. 수정 전 run `32149018524`는 생성자 누락으로 실패하고 수정 후 run `32149260760`은 통과했으며, release run `32149573778`로 beta `1.0.17` (`versionCode=21`, `status=completed`)을 배포했다. 같은 SM-S937N/Android 16에서 정상 실행을 확인한 뒤 `gplay promote` edit `01669516731482208088`로 production에 100%, `completed`로 승격했고 별도 Play 조회에서 production·beta `1.0.17`/`21`/`completed`를 확인했다. R8 코드 최적화 권장 조치를 완전히 닫으려면 DEX 게이트를 유지한 별도 beta에서 코드 최적화를 다시 켜 검증해야 한다.
   20. 다른 Android 기기의 새 설치본에서 옛 `ott.preview.pe.kr` 시절 로고가 splash에 표시되는 것을 확인했다. 런처 아이콘만 교체되고 `@drawable/splash` 5종이 최초 TWA 생성 시점에 머문 것이 원인이었다. PR `#100`, main SHA `45f75666737fa3fa080bded0ef754b239b9e7ff5`로 현재 canonical 로고와 store icon을 동기화하고 AAB의 디코딩된 ARGB 픽셀을 검사하는 게이트를 추가했다. 수동 AAB run `32334961736`과 release run `32335236662`가 통과했고, beta `1.0.18`/`22`에서 현재 로고 splash와 정상 실행을 실기기 확인했다. 이어 `gplay promote` edit `03467688027415996575`로 동일 번들을 production에 100%, `completed`로 승격했으며, 새 Play edit `08239648148585704199` 재조회에서 production·beta 모두 `1.0.18`/`22`/`completed`임을 확인했다.
...
2. 현재 판단
   1. `2026-05-23` 기준 실제 사용자 규모가 작고 복구 카드 저장 UX가 먼저 반영되어 즉시 구현은 보류한다.
   2. 사용자 증가, 코드 공유/유출 리스크, 외부 채널 심사 요구가 생기면 P1로 올려 진행한다.
3. 추후 진행안
   1. 새 코드 길이를 12자 이상으로 늘리고 표시만 `XXXX-XXXX-XXXX`처럼 그룹화한다.
   2. 입력은 대소문자, 공백, 하이픈을 정규화해 기존 사용성을 유지한다.
...
4. 문서-only 변경은 브랜치/PR 없이 main에서 직접 반영한다. 커밋 전후 `git status`로 작업트리를 확인하고, 최소 `git diff --check`를 수행한다.
...
1. `POST /api/nalytic/events`
   ...
   4. 주요 이벤트 종류: `app_open`, `title_search`, `title_select`, `login_success`, `first_log_create`, `log_create`, `guide_cta_click`, `recommendation_open`, `recommendation_refresh`, `recommendation_dismiss`, `bookshelf_open`, `bookshelf_category_open`, `android_play_cta_impression`, `android_play_cta_click`, `android_play_cta_dismiss`, `android_reminder_card_impression`, `android_reminder_card_open`, `android_reminder_card_dismiss`
   ...
   6. Android TWA 앱 접근은 `platform=twa`, `installState=twa`, `osFamily=android`로 집계한다. 판정 근거는 정확한 `android-app://app.ottline` referrer, 현재 URL의 `android_app_version`/`android_app_version_code`·`android_install_token`, 그 신호로 기록한 현재 session flag뿐이다. 일반 WebView·standalone PWA·과거 저장 컨텍스트 자체는 제외하고, `properties`에 `androidAppVersion`, `androidAppVersionCode`, `androidTwaSignal`을 저장한다.
...
7. `GET /internal/admin/report/daily`
   ...
   4. Cloudflare HTTP·RUM은 KST 전일 `[00:00, 다음 날 00:00)`을 UTC `datetime_geq`/`datetime_lt`로 변환해 같은 범위를 집계한다.
   5. Cloudflare RUM은 `visits`·`방문`으로 표시하며 고유 방문자 수로 해석하지 않는다. API의 기존 `uniqueVisitors`는 순차 배포 호환을 위해 같은 값으로 유지한다.
   6. GA4 전일 값은 조회 시점 기준 잠정치다. Kubernetes는 `generatedAt` 시점의 스냅샷이며, iOS `app_store_testflight`는 실제 설치 출처를 구분하지 않는 호환 값이다.
