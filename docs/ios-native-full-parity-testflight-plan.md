...
- `apps/native`는 현재 `main`에 편입된 Expo React Native iOS 네이티브 앱이다.
- iOS 앱은 WebView/PWA/TWA 래퍼가 아니라 사용자 앱 화면을 네이티브 화면으로 구현한다.
- Android production 경로는 계속 `apps/twa`이며, iOS 네이티브 작업 중 `apps/twa` 배포/패키지 계약을 변경하지 않는다.
- iOS bundle identifier는 `app.ottline`이고, App Store Connect Apple ID는 `6780318110`이다.
- EAS project id는 `efe8f7e5-75d8-45a9-9a4e-88bfeba07b98`이며, `apps/native/eas.json`의 TestFlight submit profile에 `ascAppId`가 반영돼 있다.
- GitHub Actions `Native iOS CI`와 수동 `Native iOS TestFlight` workflow가 동작한다.
- 로컬 작업환경은 WSL on ARM Linux이므로 iOS 빌드 성공은 로컬 검증 기준에서 제외하고, GitHub Actions와 EAS Build/Submit을 source of truth로 둔다.
- `apps/native/eas.json`의 TestFlight build profile은 `autoIncrement: true`다. 현재 workflow에서는 EAS가 submitted build number를 한 번 올리므로, App Store Connect에 올릴 실제 번호보다 committed `ios.buildNumber`를 하나 낮게 둔다.
- 최신 QA 기준 TestFlight build는 `2026-06-21` build `1.0.0 (14)`다.
- 다음 TestFlight 목표는 build `1.0.0 (15)`이며, 재시도 기준 SHA는 `76d82da9526d8ce97b7c09dda65286eef1479f26`이다. 현재 committed `ios.buildNumber`는 `14`다.
- `2026-06-21` build `15` 재시도 run `27902317186`은 EAS Free plan iOS build quota exhausted로 build/upload 전 실패했다. EAS quota reset이 `2026-07-01`로 확인되어 그때 재시도한다.
- 남은 작업은 TestFlight 설치 빌드 기준 iPhone 실기기 QA, 웹/Android 앱 대비 기능/UI/UX/문구 parity 안정화, App Store 심사 제출 준비다.
...
- 당시 남은 외부 blocker는 Apple Developer Program 활성화, App Store Connect 앱 생성, `ascAppId` 확보, iOS signing credentials/App Store Connect API key 구성이다.
...
- 당시 TestFlight 제출 전 남은 외부 blocker는 App Store Connect 앱 생성, `ascAppId` 확보, iOS signing credentials 구성, EAS Submit용 App Store Connect API key 구성이다.
- 당시 `apps/native/eas.json`의 `submit.testflight.ios.ascAppId`는 `TODO_ASC_APP_ID`였으며, App Store Connect 앱 생성 후 numeric Apple ID로 교체해야 했다.
- 당시 `ascAppId`를 채우기 전에는 `npm run native:testflight:check`가 실패하는 것이 정상이고, 구조 확인은 `npm run native:testflight:check:structure`로 수행했다.

### 진행 업데이트: 2026-06-15 App Store Connect 앱 생성

- App Store Connect 앱 레코드가 생성됐고 Apple ID `6780318110`을 확보했다.
- `apps/native/eas.json`의 `submit.testflight.ios.ascAppId`를 `6780318110`으로 교체했다.
- 당시 TestFlight 제출 전 남은 외부 blocker는 iOS signing credentials 구성, EAS Submit용 App Store Connect API key 구성, GitHub `EXPO_TOKEN` 확인이다.

### 진행 업데이트: 2026-06-18 ~ 2026-06-20 TestFlight 제출과 실기기 QA

- `2026-06-18` PR `#67` main SHA `d18cfa6`로 `apps/native`와 native iOS CI/TestFlight 경로를 `main`에 반영했다. TestFlight run `27745616062`, EAS build `455d8658-422e-4298-a023-37070d220622`로 build `1.0.0 (5)` 제출을 완료했다.
- `2026-06-19` PR `#68` main SHA `24d2845`로 하단 탭 아이콘을 `react-native-svg` 기반으로 교체하고, 설정 동기화 후 타임라인 reload 이벤트와 상단 로고/워드마크의 기록하기 탭 이동을 반영했다. TestFlight run `27804770845`로 build `1.0.0 (6)` 제출을 완료했다.
- `2026-06-19` PR `#69` main SHA `667aafeb4546eb015a9ef7894f6cba9183db043e`로 탭 화면의 큰 제목/설명은 복원하고 작은 중복 kicker 라벨만 제거했다. 페어링 코드 연결 직후 기존 `lastSyncAt` 체크포인트를 초기화해 새로 연결한 계정의 기존 타임라인을 전체 pull 하도록 수정했다. TestFlight run `27805741470`, EAS build `7796ef11-75c1-4acb-95d7-96018e10bdbc`로 build `1.0.0 (7)` 제출을 완료했다.
- build `1.0.0 (7)` 기준 검증은 `npm run native:typecheck`, `npm run native:test`, `npm run native:testflight:check`, `git diff --check`, `npx expo-doctor`, GitHub Actions typecheck/test, EAS Build, EAS Submit 모두 통과했다.
- 현재 최우선 QA는 TestFlight 설치 빌드에서 기록하기/타임라인/함께/설정의 기능, UI/UX, 문구가 웹/Android 서비스와 어긋나지 않는지 확인하는 것이다. 특히 하단 탭 아이콘/라벨, 상단 헤더, 로고 탭 이동, 설정 sync 후 타임라인 반영, 페어링 계정 전환 후 전체 pull을 우선 확인한다.

### 진행 업데이트: 2026-06-21 iOS/Web 세밀 parity 보강

- `feature/native-iphone-qa`에서 기록하기 화면을 Web QuickLog처럼 `영상/책` 2분류 검색과 작품 선택 후 상태 버튼으로 먼저 local-first 저장하고, 이후 같은 기록에 날짜/평점/플랫폼/장소/상황/시즌/에피소드/메모를 보강하는 흐름으로 정리했다.
- 빈 검색 추천은 최근 14일 함께 기록을 먼저 보여주고, 영상 필터에서 부족한 수만 `/api/titles/popular`로 채우도록 맞췄다. 책 필터에는 인기 영상 fallback을 섞지 않는다.
- 기록하기와 타임라인에서 공유 카드 생성, 함께 기록 공개를 바로 실행할 수 있게 했다. 공유 카드는 기존 native `LogShareCard`를 쓰되 포스터/시즌 포스터가 있으면 카드 상단에 실제 이미지를 렌더한다.
- 타임라인 목록 카드는 포스터, 상태/meta, 메모, sync 상태, 카드 공유/함께 공개 액션을 한 카드에서 확인하도록 보강했다.
- 제목 상세에는 해당 작품의 함께 기록으로 이동하는 진입점을 추가하고, 기록 수정의 플랫폼 입력에는 자주 쓰는 플랫폼 칩을 붙였다.
- 설정 데이터 관리에는 전체/영화/시리즈/책 범위별 CSV 내보내기를 추가했다.
- 앱이 foreground로 돌아올 때 `syncNow()`를 실행해 Web visibility sync에 대응하는 iOS 동기화 트리거를 추가했다.
- 현재 로컬 확인: `npm run native:typecheck` 통과. 남은 확인은 `npm run native:test`, `npm run native:testflight:check`, `git diff --check`, `npx expo-doctor`와 실기기 QA다.

### 진행 업데이트: 2026-06-21 App Store 제출 준비 전환

- EAS Free plan iOS build quota가 소진되어 build `1.0.0 (15)` TestFlight 재시도는 `2026-07-01` 이후로 미룬다.
- `apps/native/app.json`의 `ios.buildNumber`는 `14`로 유지한다. EAS `autoIncrement: true`가 실제 build `15`를 만들기 때문이다.
- App Store Connect metadata, privacy label, review notes, screenshot 계획, UGC moderation 리스크를 `docs/ios-app-store-submission-prep.md`에 별도 정리했다.
- 현재 iOS 공개 상세에는 공개 글/댓글 신고 진입점과 사용자 차단/뮤트 UX가 없다. 공개 글/댓글을 유지한 채 App Store 정식 심사로 가려면 Apple Guideline 1.2 대응을 P0로 처리해야 한다.
...
- `apps/native/eas.json`
  ...
  - `submit.testflight.ios.ascAppId`는 App Store Connect Apple ID `6780318110`으로 설정
...
|---|---|---:|---|
| 기록하기 | 제목 검색, 인기/최근 추천, 영상/책 2분류 검색, 상태/별점/메모/OTT/장소/상황, 시즌/에피소드, local-first create/update | 완료 | 빈 검색 추천, `/api/titles/popular`, `/api/tmdb/tv/*/seasons`, 시즌/에피소드 선택, create/update outbox까지 구현 |
...
- 공개 글/댓글/멘션은 UGC이므로 신고/필터링/차단/운영 연락 수단이 필요하다. 현재 iOS 공개 상세에는 공개 글/댓글 신고 진입점과 사용자 차단/뮤트 UX가 없으므로, App Store 정식 심사 전 P0 리스크로 처리한다. 완료 전에는 App Store Connect review notes에 신고/차단 기능이 있다고 쓰지 않는다. 출처: https://developer.apple.com/app-store/review/guidelines/
...
- privacy nutrition label, support URL, marketing URL, privacy URL, screenshots는 App Store 정식 제출 전 필수다. TestFlight 입력 초안은 `docs/ios-testflight-review-notes.md`, App Store 제출 준비 초안은 `docs/ios-app-store-submission-prep.md`에 둔다.
...
- App Store metadata/privacy/review/screenshot 초안은 `docs/ios-app-store-submission-prep.md` 기준으로 갱신
- 공개 글/댓글 UGC 신고/차단 리스크를 정식 심사 제출 전에 해결
...
- 완료: App Store Connect/EAS 외부 준비
- 완료: `eas.json` submit profile 완성
- 완료: GitHub TestFlight workflow 추가
- 완료: workflow_dispatch로 SHA 지정 build
- 완료: EAS Build 성공
- 완료: EAS Submit 성공
- 완료: App Store Connect TestFlight build 업로드
- 진행 중: build `1.0.0 (14)` 내부 테스터 설치와 iPhone 실기기 QA
- 대기: EAS Free plan quota reset 이후 build `1.0.0 (15)` 재시도

완료 증거:

- Last known successful evidence: build `1.0.0 (14)` is the current TestFlight QA baseline.
- Next retry evidence target: SHA `76d82da9526d8ce97b7c09dda65286eef1479f26`, intended App Store Connect build `1.0.0 (15)`.
- Failed retry evidence: GitHub run `27902317186` stopped before build/upload because EAS Free plan iOS build quota was exhausted.
- iPhone 설치 후 핵심 플로우 확인표: 진행 중
...
- `npm run native:testflight:check`가 성공한다.
...
- 내부 테스터가 TestFlight에서 설치할 수 있다. (진행 중)
- iPhone 실기기 확인표의 필수 항목이 통과했다. (진행 중)
- TestFlight/App Review 리스크 중 review access, privacy/support URL, UGC moderation gap에 대한 대응이 문서화됐다. UGC 신고/차단 UX는 App Store 정식 심사 전 P0로 남아 있다.
