...
2. 현재 기준
   ...
   3. `2026-06-07` 기준 Google Play `production` 최신 버전은 `1.0.10` (`versionCode=14`, `status=completed`)이다. `1.0.10`은 Android 회고 리마인드(주간 회고, 월간 장르, 7일 기록 공백, 시리즈 이어보기)와 웹 `/me/report` 주간/월간 묶음 공유카드를 추가한 업데이트다. 시청 기록 알림은 Usage Access 기반으로 대상 OTT 앱 사용 종료 후 기록 알림을 띄우는 네이티브 설정/스케줄러를 TWA release 앱에 포함한다. 콘텐츠 제목/재생 상태/화면 내용은 읽지 않는다.
