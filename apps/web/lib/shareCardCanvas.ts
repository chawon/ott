...
export async function renderShareCardBlobLocally(payload: ShareCardPayload) {
  ...
  const profileNickname =
    payload.showProfileSignature && payload.profileNickname?.trim()
      ? clampText(payload.profileNickname.trim(), 32)
      : null;
  ...
  let profileAvatarImage: HTMLImageElement | null = null;
  if (profileNickname && payload.profileAvatarUrl) {
    try {
      profileAvatarImage = await loadImage(payload.profileAvatarUrl);
    } catch {
  ...
  const titleLineHeight = Math.round(
    (isBook ? s(64) : s(84)) * (isBook ? 1.12 : 1.05),
  );
  ...
  const footerY = height - s(104);
  if (profileNickname) {
    const avatarSize = s(54);
    if (profileAvatarImage) {
      drawCircularImage(ctx, profileAvatarImage, paddingX, footerY, avatarSize);
    }
    ctx.fillStyle = "rgba(255,255,255,0.9)";
    ctx.font = `${s(28)}px sans-serif`;
    ctx.fillText(profileNickname, paddingX + avatarSize + s(14), footerY);
    ctx.fillStyle = "rgba(255,255,255,0.62)";
    ctx.font = `${s(22)}px sans-serif`;
    ctx.fillText(
      "by ottline.app",
      paddingX + avatarSize + s(14),
      footerY + s(32),
    );
  } else {
