...
35. Android TWA 시작 화면 브랜드 동기화: 최초 TWA 생성 시점의 옛 splash PNG 5종과 store icon을 canonical 현재 ottline 로고로 교체하고, AAB 안의 이미지를 디코딩해 모든 픽셀을 검사하는 CI 게이트를 추가했다. `2026-08-20` beta `1.0.18` (`versionCode=22`, `status=completed`) 배포를 완료했고 production은 실기기 확인 전까지 `1.0.17` (`versionCode=21`)을 유지한다.
...
2. 현재 기준
   ...
   3. `2026-08-20` 기준 Google Play `beta` 최신 버전은 `1.0.18` (`versionCode=22`, `status=completed`), `production`은 `1.0.17` (`versionCode=21`, `status=completed`), `internal`은 `1.0.14` (`versionCode=18`, `status=completed`)이다. 시청 기록 알림은 Usage Access 기반으로 대상 OTT 앱 사용 종료 후 기록 알림을 띄우며 콘텐츠 제목·재생 상태·화면 내용은 읽지 않는다. 다음 Play 업데이트는 `versionCode=23` 이상을 사용한다.
