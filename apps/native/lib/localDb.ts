...
type HistoryRow = {
  ...
  season_poster_url: string | null;
  season_year: number | null;
  origin: WatchLogHistory['origin'] | null;
  watched_at: string;
  place: WatchLogHistory['place'] | null;
  occasion: WatchLogHistory['occasion'] | null;
};
...
function rowToHistory(row: HistoryRow): WatchLogHistory {
  return {
    id: row.id,
    logId: row.log_id,
...
export async function initDb() {
  ...
  await db.execAsync(`
    ...
    CREATE INDEX IF NOT EXISTS idx_history_log_recorded_at ON history(log_id, recorded_at);
...
export async function listLogsByTitleLocal(titleId: string) {
  await initDb();
  const db = await getDb();
  const rows = await db.getAllAsync<LogRow>(
    'SELECT * FROM logs WHERE title_id = ? AND deleted_at IS NULL ORDER BY watched_at DESC, created_at DESC',
    titleId,
  );
  return rows.map(rowToLog);
}
...
export async function countLogsLocal() {
  ...
  const row = await db.getFirstAsync<{ count: number }>(
    'SELECT COUNT(*) as count FROM logs WHERE deleted_at IS NULL',
  );
  return row?.count ?? 0;
}

export async function getLogLocal(id: string) {
  await initDb();
  const db = await getDb();
...
export async function upsertHistoryLocal(items: WatchLogHistory[]) {
  ...
  for (const item of items) {
    await db.runAsync(
      `
        INSERT INTO history(
          id, log_id, recorded_at, status, rating, note, spoiler, ott,
          season_number, episode_number, season_poster_url, season_year, origin,
          watched_at, place, occasion
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
...
export async function listHistoryByLogLocal(logId: string, limit = 50) {
  await initDb();
  const db = await getDb();
  const rows = await db.getAllAsync<HistoryRow>(
    'SELECT * FROM history WHERE log_id = ? ORDER BY recorded_at DESC LIMIT ?',
    logId,
    limit,
  );
...
export async function enqueueUpdateLogOutbox(log: WatchLog, payload: LogOutboxPayload) {
  await initDb();
  const db = await getDb();
  const pendingCreate = await db.getFirstAsync<OutboxRow>(
    'SELECT * FROM outbox WHERE log_id = ? ORDER BY created_at ASC LIMIT 1',
    log.id,
  );
  ...
  await enqueueLogOutbox(log, payload);
}

export async function markLogDeletedLocal(
  logId: string,
  ...
  const now = new Date().toISOString();
  await db.runAsync(
    'UPDATE logs SET deleted_at = ?, updated_at = ?, sync_status = ? WHERE id = ?',
    deletedAt ?? now,
...
export async function deleteLogLocal(logId: string) {
  const existing = await getLogLocal(logId);
  if (!existing) return null;
  const now = new Date().toISOString();
  const deleted: WatchLog = {
    ...existing,
    deletedAt: now,
    updatedAt: now,
    syncStatus: 'pending',
  };
  await upsertLogLocal(deleted);
  return deleted;
}
