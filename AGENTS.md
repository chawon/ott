...
2. 현재 기준
   ...
   4. Google Play 프로덕션 액세스는 `2026-06-05` 기준 비공개 테스트 14일 조건을 완료하고 프로덕션 심사 제출까지 진행했으며, 아직 심사 결과는 나오지 않았다. 이전 Play 검토 반려 사유는 비공개 테스트 중 실제 테스터 참여가 없었고, 사용자 의견 수집/조치/앱 업데이트 같은 테스트 권장사항을 충족하지 못했다는 점이었다. `2026-05-28` `gplay` 재확인 기준 production 트랙 릴리즈는 없고, `alpha` 테스터 그룹은 `ottline-beta-testers@googlegroups.com`가 연결돼 있었다.
   ...
   6. `2026-05-21` 기준 `1.0.6` (`versionCode=10`)을 Google Play `alpha`에 업로드해 비공개 테스트를 다시 시작했다. 12명 이상 테스터가 모두 opt-in한 시점부터 14일 연속 조건을 계산하며, `2026-06-05` 기준 14일 조건 완료 후 프로덕션 심사 제출까지 진행했다.
