...
2. 현재 기준
   ...
   7. `2026-05-31` 기준 `1.0.8` (`versionCode=12`)을 Google Play `alpha`에 업로드했다. PR `#40`, main SHA `e6b5cfa`, web staging run `26712369692`, TWA release run `26712381951`이며 `gplay tracks get`으로 `alpha`의 `1.0.8`/`12`/`completed` 반영을 확인했다. Debug APK에는 감지 진단 UI가 남아 있지만 release/AAB 설정 화면은 사용 정보 접근, 알림 권한, 알림 켜기 순서의 사용자용 3단계 UI만 노출한다.
