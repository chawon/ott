...
export function buildUpdateLogPayload(log: WatchLog, updatedAt: string): LogOutboxPayload {
  return buildOutboxPayload(log, updatedAt);
}

export function buildDeleteLogPayload(log: WatchLog, updatedAt: string): LogOutboxPayload {
  return {
    title: buildTitleChange(log.title, updatedAt),
    log: {
      id: log.id,
