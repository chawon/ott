...
## 2. 구현 상세
### 2.1. 마이그레이션 트리거 (`MigrationBanner.tsx`)
...
- **Ingress 유지**: 구 도메인과 신규 도메인의 TLS 설정을 각각 독립적으로 유지하여 병행 운영 가능하도록 조치.

## 4. 향후 계획
- **차등 브랜딩**: `ottline.app` 도메인으로 접속 시에만 신규 브랜드(로고, 명칭 등) 적용.
