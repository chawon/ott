...
- `/privacy`, `/en/privacy` 같은 개인정보 처리방침 진입은 제품 실행으로 세지 않고, 언어별 문서 경로당 세션 1회의 `public_page_view`로 분리한다. 공개 문서 이벤트는 제품 유입용 `landingPath`, referrer, UTM 세션 상태를 만들지 않으므로 이후 기록 화면으로 이동하면 그 시점의 경로로 정상 `app_open`을 보낸다.
...
유입 영역은 채널, 출처, 첫 진입 경로, 캠페인 순으로 같은 세션·행동·첫 기록·기록 생성 열을 제공한다. `source`는 UTM, 자체 재방문 진입, 외부 referrer를 정규화한 실제 출처이며 채널보다 상세한 유입 판단에 사용한다.
