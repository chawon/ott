...
2. 현재 기준
   ...
   3. `2026-08-18` 기준 Google Play `production` 최신 버전은 `1.0.13` (`versionCode=17`, `status=completed`)이고, `internal`과 `beta` 최신 버전은 `1.0.14` (`versionCode=18`, `status=completed`)이다. `1.0.14`는 Android 활성화 2차, Android 15 edge-to-edge, 지원 중단 system-bar API 대응, R8 코드·리소스 최적화를 포함한다. 시청 기록 알림은 Usage Access 기반으로 대상 OTT 앱 사용 종료 후 기록 알림을 띄우며 콘텐츠 제목·재생 상태·화면 내용은 읽지 않는다.
