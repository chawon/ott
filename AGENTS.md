...
2. 현재 기준
   ...
   3. `2026-08-19` 기준 Google Play `production`과 `beta` 최신 버전은 `1.0.17` (`versionCode=21`, `status=completed`)이고 `internal`은 `1.0.14` (`versionCode=18`, `status=completed`)이다. 시청 기록 알림은 Usage Access 기반으로 대상 OTT 앱 사용 종료 후 기록 알림을 띄우며 콘텐츠 제목·재생 상태·화면 내용은 읽지 않는다. 다음 Play 업데이트는 `versionCode=22` 이상을 사용한다.
