...
이 계획의 원래 완료 기준은 App Store 정식 출시가 아니라, `main` 기준 코드에 네이티브 iOS 앱을 안전하게 편입하고 GitHub Actions에서 iOS 빌드와 TestFlight 제출을 재현 가능하게 만드는 것이었다. 이후 QA parity와 App Store 제출 준비가 이어졌고, `2026-06-29` App Store 심사 통과 및 정식 공개까지 완료됐다.
...
- `apps/native/app.json`의 committed `ios.buildNumber`는 다음 제출 baseline이며, EAS `autoIncrement: true`가 App Store Connect 업로드 시 build number를 1씩 올린다.
- App Store 출시 후보는 build `1.0.0 (18)`이며, `2026-06-29` App Store 심사를 통과해 `https://apps.apple.com/app/ottline/id6780318110`로 공개됐다.
- 출시 후 남은 확인은 실제 App Store 설치본 실행, 신규 설치 첫 기록, 페어링 코드 연결, production analytics `platform=ios_native` 유입 확인이다. 상세 항목은 `docs/ios-app-store-launch-checklist.md`를 기준으로 한다.
...
### 진행 업데이트: 2026-06-28 ~ 2026-06-29 App Store 출시

- `native-ios-qa-parity` 브랜치에서 iOS QA parity와 출시 준비를 마무리했고, PR `#73` main SHA `2ddb3bb`로 `main`에 반영했다.
- Expo 기본 아이콘이 들어간 build `1.0.0 (16)`은 출시 후보에서 제외하고, ottline 브랜드 아이콘을 반영한 build `1.0.0 (18)`을 App Store Connect에 업로드했다.
- `2026-06-29` App Store 심사를 통과해 App Store에 공개됐다. 공개 링크는 `https://apps.apple.com/app/ottline/id6780318110`이다.
...
- Android 네이티브 패키지 전환
- WebView 기반 화면 재사용
- App Store 정식 심사 제출과 출시는 이 계획의 원래 구현 범위 밖이었지만, 후속 launch prep으로 `2026-06-29` 완료됐다.
...
- 완료: App Store Connect TestFlight build processing/available 확인
- 완료: 내부 테스터 설치와 iPhone 실기기 QA
- 완료: App Store 심사 통과 및 정식 공개

완료 증거:

- GitHub run URL: `https://github.com/chawon/ott/actions/runs/27805741470`
- EAS build URL: `https://expo.dev/accounts/ergate23/projects/ottline/builds/7796ef11-75c1-4acb-95d7-96018e10bdbc`
- EAS submission URL: `https://expo.dev/accounts/ergate23/projects/ottline/submissions/2f02d6c8-2200-4078-b40b-b4ee0591bc54`
- App Store Connect build number: `1.0.0 (18)`
- TestFlight processing/available 상태: 완료
- iPhone 설치 후 핵심 플로우 확인표: `2026-06-28` 사용자 확인 완료
